...
async function loadConfig() {
    config = {discord_invite: 'https://discord.gg/example'};
    return config;
}
...
function updateUserProfile(user) {
    const usernameDisplays = document.querySelectorAll('#usernameDisplay, #usernameDisplaySidebar');
    usernameDisplays.forEach(el => el.textContent = user.username || 'Guest');

    const userType = user.account_type || 'User';
    const userTypeElems = document.querySelectorAll('#userType, #userTypeSidebar');
    userTypeElems.forEach(el => el.textContent = userType);

    const profileImages = document.querySelectorAll('#profileImage, #profileImageLarge');
    profileImages.forEach(img => img.src = user.profile_picture || '');
}

async function loginUser(usernameOrEmail, password, rememberMe) {
    try {
        const response = await fetch('https://prem-eu4.bot-hosting.net:20940/api/login', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            sessionStorage.setItem('rickware_refresh_token', data.refresh_token);
...
async function registerUser(username, email, phone, password, newsletter) {
    try {
        const response = await fetch('https://prem-eu4.bot-hosting.net:20940/api/register', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
